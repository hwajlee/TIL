...
class Feed(models.Model):
    ...
    # related_name -> reverse access의 이름을 바꿔줌 p1.feed_set.all() -> p1.feeds.all()
...
p1.like_feeds.all(f1) # p1이 좋아요 한 게시글에 f1 추가 => join 테이블에 p1, f1 레코드 추가 
f1.like_people.add(p1) # 결과 같음(데이터 중복 안됨)

pl.like_feeds.remove(f1) # join 테이블에서 p1, f1으로 구성된 레코드 삭제 
